...
func NewCertManager(tunnel TunnelCertificate, nodeName string) *CertManager {
	...
	return &CertManager{
		RotateCertificates: tunnel.RotateCertificates,
		NodeName:           nodeName,
		token:              tunnel.Token,
		CR:                 certReq,
...
func GetPrivateKey(config TunnelCertificate, nodeName string) (privatekey crypto.PrivKey, err error) {
	certManager := NewCertManager(config, nodeName)
	certManager.Start()

	certBytes, err := ioutil.ReadFile(config.TLSPrivateKeyFile)
	if err != nil {
		return privatekey, fmt.Errorf("read file %s err: %v", config.TLSPrivateKeyFile, err)
	}
