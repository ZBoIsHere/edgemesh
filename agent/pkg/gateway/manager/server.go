...
import (
	...
	"github.com/kubeedge/edgemesh/agent/pkg/common/chassis/protocol"
	"github.com/kubeedge/edgemesh/agent/pkg/common/chassis/protocol/http"
	"github.com/kubeedge/edgemesh/agent/pkg/common/chassis/protocol/tcp"
	...
	for {
		...
		if err != nil {
			...
			if srv.options.CredentialName != "" {
				...
				s, err := lister.GetSecretLister().Secrets(srv.options.Namespace).Get(srv.options.CredentialName)
				if err != nil {
...
func (srv *Server) newProto(conn net.Conn) (protocol.Protocol, error) {
	...
	if srv.options.Exposed {
		...
		vsList, err := lister.GetVsLister().VirtualServices(srv.options.Namespace).List(labels.Everything())
		if err != nil {
