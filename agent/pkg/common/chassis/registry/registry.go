...
func (esd *EdgeServiceDiscovery) FindMicroServiceInstances(consumerID, microServiceName string, tags utiltags.Tags) ([]*registry.MicroServiceInstance, error) {
	...
	// get service
	svc, err := lister.GetSvcLister().Services(namespace).Get(name)
	if err != nil {
		return nil, err
	}
	if svc == nil {
	...
	// get endpoints
	eps, err := lister.GetEpLister().Endpoints(namespace).Get(name)
	if err != nil {
		return nil, err
	}
	if eps == nil {
	...
	// get pods
	pods, err := lister.GetPodLister().Pods(namespace).List(util.GetPodsSelector(svc))
	if err != nil {
		return nil, err
	}
	if len(pods) == 0 {
