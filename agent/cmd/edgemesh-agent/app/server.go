...
import (
	...
	"github.com/kubeedge/edgemesh/agent/pkg/common/chassis"
...
func Run(cfg *config.EdgeMeshAgentConfig) error {
	...
	klog.Infof("[%d] Init resource lister", trace)
	lister.Init(ifm)
	trace++
	...
	if plugin != nil {
		klog.Infof("[%d] Run go-chassis plugins", trace)
		plugin.Run()
		trace++
	}
